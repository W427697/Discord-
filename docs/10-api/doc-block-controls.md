...
The Controls doc block will only have functioning UI controls if you have also installed and registered [`@storybook/addon-controls`](../06-essentials/controls.md) (included in [`@storybook/addon-essentials`](../06-essentials/index.md)).
...
ℹ️ Like most blocks, the `Controls` block is configured with props in MDX. Many of those props derive their default value from a corresponding [parameter](../02-writing-stories/parameters.md) in the block's namespace, `parameters.docs.controls`.
...
This API configures Controls blocks used within docs pages. To configure the Controls addon panel, see the [Controls addon docs](../06-essentials/controls.md). To configure individual controls, you can specify [argTypes](./arg-types.md#control) for each.
