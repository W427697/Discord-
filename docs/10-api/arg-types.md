...
You can also use argTypes to “annotate” args with information used by addons that make use of those args. For instance, to instruct the [controls addon](../06-essentials/controls.md) to render a color picker, you could specify the `'color'` [control type](#control).
...
If you are using the Storybook [docs](../03-writing-docs/index.md) addon (installed by default as part of [essentials](../06-essentials/index.md)), then Storybook will infer a set of argTypes for each story based on the `component` specified in the [default export](../02-writing-stories/index.md#default-export) of the CSF file.
...
ArgTypes are most often specified at the meta (component) level, in the [default export](../02-writing-stories/index.md#default-export) of the CSF file:
...
Or they can apply only to a [specific story](../02-writing-stories/index.md#defining-stories):
...
Specify the behavior of the [controls addon](../06-essentials/controls.md) for the arg. If you specify a string, it's used as the [`type`](#controltype) of the control. If you specify an object, you can provide additional configuration. Specifying `false` will prevent the control from rendering.
...
Specifies the type of control used to change the arg value with the [controls addon](../06-essentials/controls.md). Here are the available types, `ControlType`, grouped by the type of data they handle:
...
Conditionally render an argType based on the value of another [arg](../02-writing-stories/args.md) or [global](../06-essentials/toolbars-and-globals.md).
...
Specify how the arg is documented in the [`ArgTypes` doc block](./doc-block-argtypes.md), [`Controls` doc block](./doc-block-controls.md), and [Controls addon panel](../06-essentials/controls.md).
...
Define the default value of the argType. Deprecated in favor of defining the [`arg`](../02-writing-stories/args.md) value directly.
