...
export default class StoryStore {
  ...
  getStoryKinds() {
    return Object.keys(this._data).map(key => this._data[key]).filter(kind => Object.keys(kind.stories).length > 0).sort((info1, info2) => info1.index - info2.index).map(info => info.kind);
  }
  ...
  getStories(kind) {
    ...
    return Object.keys(this._data[kind].stories).map(name => this._data[kind].stories[name]).sort((info1, info2) => info1.index - info2.index).map(info => info.name);
  }
