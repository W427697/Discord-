...
const linkCommand = `nx run-many --target="prep" --all --parallel --max-parallel=${maxConcurrentTasks} --exclude=@junk-temporary-prototypes/addon-storyshots,@junk-temporary-prototypes/addon-storyshots-puppeteer -- --reset`;
...
export const compile: Task = {
  ...
  async ready({ codeDir }, { link }) {
    try {
      ...
      // `@junk-temporary-prototypes/preview`. To check if it has been built for publishing (i.e. `--no-link`),
