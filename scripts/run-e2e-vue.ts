...
const defaultVueCliVersion = 'latest';

const parameters = {
  name: 'vue',
  version: 'latest',
  generator: `
    npx -p @vue/cli@{{version}} vue create {{name}}-v{{version}} --default
  `,
};
...
const initStorybook = async ({ cwd, name }: Options) => {
  ...
  try {
    await exec(`npx -p @storybook/cli sb init --type ${name}`, { cwd });
  } catch (e) {
...
let vueCliVersions = process.argv.slice(2);

if (!vueCliVersions || vueCliVersions.length === 0) {
  vueCliVersions = [defaultVueCliVersion];
}
