...
const recursiveRemove = (stories, id) => {
  const node = stories[id];
  // eslint-disable-next-line no-param-reassign
  delete stories[id];
  if (node.parent) {
    const parent = stories[node.parent];
    parent.children = parent.children.filter(child => child !== id);
    if (!parent.children.length) {
      recursiveRemove(stories, parent.id);
    }
  }
};

export const removeDocsOnlyStories = stories => {
  const filtered = cloneDeep(stories);
  Object.values(stories).forEach(({ id, parameters }) => {
    if (parameters && parameters.docsOnly) {
      recursiveRemove(filtered, id);
    }
  });
  return filtered;
};

export const flattenDocsOnlyStories = stories => {
  const filtered = cloneDeep(stories);
  Object.values(stories).forEach(story => {
    ...
    if (isLeaf && parameters && parameters.docsOnly) {
      if (!filtered[parent].isLeaf) {
        ...
        filtered[parent].id = id;
        delete filtered[parent].children;
      }
      delete filtered[id];
    }
  });
...
export const mapper = ({ state, api }) => {
  ...
  const stories = docsMode
    ? memoize(1)(removeLeafStories)(storiesHash)
    : memoize(1)(flattenDocsOnlyStories)(storiesHash);
