...
import { loadStories, toggleFullScreen, toggleShortcutsHelp, toggleSearchBox } from './actions';
...
export default function(domNode, provider) {
  ...
  const pluginReducers = plugins.filter(plugin => plugin.reduce).map(plugin => plugin.reduce);

  const store = createAppStore(pluginMiddlwares, pluginReducers);

  const keys = new Mousetrap(document);
  keys.bind('?', () => {
    store.dispatch(toggleShortcutsHelp());
  });
  keys.bind('mod+shift+f', () => {
    store.dispatch(toggleFullScreen());
  });
  keys.bind('mod+shift+p', () => {
    store.dispatch(toggleSearchBox());
  });
  ...
  const tmpPreview = plugins.filter(plugin => plugin.decoratePreview).reduce((acc, plugin) => {
    return plugin.decoratePreview(acc, { React });
  }, provider.renderPreview);

  const mapPreviewState = plugins.filter(p => p.mapPreviewState).map(p => p.mapPreviewState);

  const Preview = connect(state => mapPreviewState.reduce((s, mapper) => mapper(state, s), {}))(
    tmpPreview
  );
