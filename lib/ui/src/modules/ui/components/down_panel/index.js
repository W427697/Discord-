...
class DownPanel extends Component {
  constructor(props) {
    super(props);
    this.style = styleFn(props.theme);
  }

  componentWillReceiveProps(nextProps) {
    this.style = styleFn(nextProps.theme);
  }

  renderTab(name, panel) {
    let tabStyle = this.style.tablink;
    if (this.props.selectedPanel === name) {
      tabStyle = Object.assign({}, this.style.tablink, this.style.activetab);
    }
  ...
  render() {
    ...
    return (
      <div style={this.style.wrapper}>
        <div style={this.style.tabbar} role="tablist">{this.renderTabs()}</div>
        <div style={this.style.content}>{this.renderPanels()}</div>
      </div>
    );
  }
}
