import { decorators } from 'react-treebeard';
import { IoChevronRight } from 'react-icons/lib/io';
import React from 'react';
import PropTypes from 'prop-types';

const highLightText = {
  backgroundColor: '#FFFEAA',
  fontWeight: 'inherit',
};

function ToggleDecorator({ style }) {
  const { height, width, arrow } = style;

  return (
    <div style={style.base}>
      <div style={style.wrapper}>
        <IoChevronRight height={height} width={width} style={arrow} />
      </div>
    </div>
  );
}

ToggleDecorator.propTypes = {
  style: PropTypes.shape({
    width: PropTypes.number.isRequired,
    height: PropTypes.number.isRequired,
    arrow: PropTypes.object.isRequired,
  }).isRequired,
};

function ContainerDecorator(props) {
  const { node } = props;

  if (node.root) {
    return null;
  }

  return <decorators.Container {...props} />;
}

ContainerDecorator.propTypes = {
  node: PropTypes.shape({
    root: PropTypes.bool,
  }).isRequired,
};

function createHeaderDecoratorScope(parent) {
  class HeaderDecorator extends React.Component {
    constructor(...args) {
      super(...args);
      this.onKeyDown = this.onKeyDown.bind(this);
    }

    onKeyDown(event) {
      const { onKeyDown } = parent;
      const { node } = this.props;

      onKeyDown(event, node);
    }

    // Prevent focusing on mousedown
    onMouseDown(event) {
      event.preventDefault();
    }

    decorateNameMatchedToSearchTerm(node) {
      const { storyFilter, name } = node;

      if (!storyFilter) {
        return name;
      }

      const filterRegex = new RegExp(`(${storyFilter})`, 'i');
      const nameParts = name.split(filterRegex);

      return nameParts.filter(part => part).map((part, index) => {
        const key = `${part}-${index}`;

        if (!filterRegex.test(part)) {
          return (
            <span key={key}>
              {part}
            </span>
          );
        }

        return (
          <strong key={key} style={highLightText}>
            {part}
          </strong>
        );
      });
    }

    render() {
      const { style, node } = this.props;

      const newStyleTitle = {
        ...style.title,
      };

      if (!node.children || !node.children.length) {
        newStyleTitle.fontSize = '13px';
      }

      const name = this.decorateNameMatchedToSearchTerm(node);

      return (
        <div
          style={style.base}
          role="menuitem"
          tabIndex="0"
          onKeyDown={this.onKeyDown}
          onMouseDown={this.onMouseDown}
        >
          <a style={newStyleTitle}>
            {name}
          </a>
        </div>
      );
    }
  }

  HeaderDecorator.propTypes = {
    style: PropTypes.shape({
      title: PropTypes.object.isRequired,
      base: PropTypes.object.isRequired,
    }).isRequired,
    node: PropTypes.shape({
      name: PropTypes.string.isRequired,
      storyFilter: PropTypes.string,
    }).isRequired,
  };

  return HeaderDecorator;
}

export default function(parent) {
  return {
    ...decorators,
    Header: createHeaderDecoratorScope(parent),
    Container: ContainerDecorator,
    Toggle: ToggleDecorator,
  };
}
