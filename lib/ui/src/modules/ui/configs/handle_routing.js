...
export function getUrlState(data) {
  ...
  const path = params.path || '/components/';
  const match = matchPath('/components', {
    path,
  });

  // ad-hoc solution until better central routing is implemented
  const url = match ? `?${qs.stringify(urlObj)}` : window.location.search;
