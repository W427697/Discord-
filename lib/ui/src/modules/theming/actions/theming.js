...
function setTheme(theme) {
  const themeName = theme.name || `Theme ${themesStore.keys.length}`;
  themesStore.set(themeName, { ...theme, name: themeName });
  return theme;
}
...
export default {
  ...
  getTheme({ clientStore }) {
    const themeName = clientStore.get('uiOptions').currentTheme;
