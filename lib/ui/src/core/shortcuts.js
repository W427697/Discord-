...
import { createKeybinding } from '../keyboard/keyCodes';
  ...
  const api = {
    ...
    getShortcutKeys() {
      const shortcuts = Object.entries(store.getState().shortcuts).reduce(
        (acc, i) => ({ ...acc, [i[0]]: createKeybinding(+i[1]) }),
        {}
      );
      return shortcuts;
    },
    async setShortcuts(shortcuts) {
      console.log('before setShortcuts shortcuts: ', shortcuts);
      await store.setState({ shortcuts }, { persistence: 'permanent' });
      console.log('after setShortcuts shortcuts: ', shortcuts);
      return shortcuts;
    },
