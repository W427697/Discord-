...
export default function initShortcuts({ store }) {
  const api = {
    ...
    async setShortcuts(shortcuts) {
      console.log('setShortcuts param shortcuts: ', shortcuts);
      await store.setState({ shortcuts }, { persistence: 'permanent' });

      console.log('setShortcuts output shortcuts: ', shortcuts);
      return shortcuts;
    },
    async restoreAllDefaultShortcuts() {
      console.log('restoreAllDefaultShortcuts api.setShortcuts(defaultShortcuts);: ', api.setShortcuts(defaultShortcuts));
      return api.setShortcuts(defaultShortcuts);
    },
    async setShortcut(action, value) {
      const shortcuts = api.getShortcutKeys();
      await api.setShortcuts({ ...shortcuts, [action]: value.getHashCode() });
      return value;
    },
