...
const renderIframe = (storyId, id, baseUrl) => {
  const start = `${baseUrl}?id=${storyId}&channel=${id}`;
  const src = !id.includes('notes') ? `${start}&sync` : start;

  return <IFrame key="iframe" id="storybook-preview-iframe" title={id} src={src} />;
};
