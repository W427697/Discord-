...
class PanelGroup extends Component {
  ...
  componentWillReceiveProps(nextProps) {
    ...
    const { panelWidths } = nextProps;

    // debugger; // eslint-disable-line

    // Only update from props if we're supplying the props in the first place
    if (panelWidths && panelWidths.length) {
      // if the panel array is a different size we know to update
      if (panels.length !== panelWidths.length) {
        this.setState(this.loadPanels(nextProps));
      } else {
        // otherwise we need to iterate to spot any difference
        for (let i = 0; i < panelWidths.length; i += 1) {
          if (
            panels[i].size !== panelWidths[i].size ||
            panels[i].minSize !== panelWidths[i].minSize ||
            panels[i].resize !== panelWidths[i].resize
          ) {
            this.setState(this.loadPanels(nextProps));
            break;
          }
  ...
  render() {
    ...
    for (let i = 0; i < initialChildren.length; i += 1) {
      ...
      newChildren.push(
        <Panel
          style={panelStyle}
          key={`panel${initialChildren[i].props.id}`}
          panelID={i}
          {...metadata}
      ...
      if (i < initialChildren.length - 1) {
        newChildren.push(
          <Divider
            ...
            key={`divider${initialChildren[i].props.id}`}
            panelID={i}
