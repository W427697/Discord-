...
class TreeState extends PureComponent {
  ...
  render() {
    const {
      ...
      RootTitle,
      events,
      props: { dataset: input, selectedId, shortcutKeys },
    ...
    const shortcutSymbol = shortcutKeys.filter(k => k.title === 'Search');
    return (
      <Fragment>
        {Filter ? (
          <Filter
            shortcutSymbol={shortcutSymbol[0].detail}
            key="filter"
...
TreeState.propTypes = {
  ...
  shortcutKeys: PropTypes.arrayOf(PropTypes.shape(keyShape)).isRequired,
};
