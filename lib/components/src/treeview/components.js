...
export class Filter extends Component {
  static propTypes = {
    shortcutSymbol: PropTypes.string.isRequired,
  };

  constructor(props) {
    super(props);
    const { shortcutSymbol } = props;

    this.state = {
      placeholder: `Press "${shortcutSymbol}" to search...`,
    };
  }

  set = e => {
    const { shortcutSymbol } = this.props;
    this.setState({
      placeholder:
        e.type === 'focus' ? 'Type to search...' : `Press "${shortcutSymbol}" to search...`,
    });
  };
