...
it('makes a fetch request with name and data', async () => {
  fetchMock.mockClear();
  expect(await sendTelemetry('test', { foo: 'bar' })).toBe(true);
...
it('retries if fetch fails with a 503', async () => {
  ...
  expect(await sendTelemetry('test', { foo: 'bar' }, { delay: 0 })).toBe(true);
...
it('gives up if fetch repeatedly fails', async () => {
  ...
  expect(await sendTelemetry('test', { foo: 'bar' }, { delay: 0 })).toBe(false);
