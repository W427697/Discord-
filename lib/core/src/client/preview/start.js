...
export default function start(render, { decorateStory } = {}) {
  ...
  const renderMain = forceRender => {
    ...
    switch (viewMode) {
      ...
      case 'story':
      default: {
        // If this is a docs-only story, fall through to the missing case.
        // This is because the user entered a docs-only story URL in story mode.
        // This can happen by navigating to that story in docs mode and then
        // clicking back to the canvas tab.
        if (getDecorated && (!parameters || !parameters.docsOnly)) {
