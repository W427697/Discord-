...
function showError({ title, description }) {
  addons.getChannel().emit(STORY_ERRORED, { title, description });
  showErrorDisplay({
    message: title,
    stack: description,
  });
...
function showException(exception) {
  addons.getChannel().emit(STORY_THREW_EXCEPTION, exception);
...
export default function start(render, { decorateStory } = {}) {
  ...
  const renderMain = forceRender => {
    ...
    if (
      ...
      addons.getChannel().emit(STORY_UNCHANGED, {
        id,
        revision,
        kind,
        name,
    ...
    if (!forceRender && previousKind && previousStory && revision === previousRevision) {
      addons.getChannel().emit(STORY_CHANGED, id);
    }
    ...
    switch (viewMode) {
      case 'docs': {
        ...
        ReactDOM.render(
          ...
          () => addons.getChannel().emit(DOCS_RENDERED, kind)
        );
        break;
      }
      case 'story':
      default: {
        if (getDecorated) {
          ...
          showNopreview();
          addons.getChannel().emit(STORY_MISSING, id);
        }
        break;
      }
    }
  ...
  if (isBrowser) {
    ...
    channel.on(FORCE_RE_RENDER, forceReRender);
    channel.on(SET_CURRENT_STORY, ({ storyId: inputStoryId, name, kind, viewMode }) => {
  ...
  storyStore.on(STORY_INIT, () => {
    const { storyId, viewMode } = initializePath();
    storyStore.setSelection({ storyId, viewMode });
  });

  storyStore.on(STORY_RENDER, renderUI);
