...
function showError({ title, description }) {
  channels.getChannel().emit(Events.STORY_ERRORED, { title, description });
  showErrorDisplay({
    message: title,
    stack: description,
  });
...
function showException(exception) {
  channels.getChannel().emit(Events.STORY_THREW_EXCEPTION, exception);
  ...
  const renderUI = forceRender => {
    if (isBrowser) {
      ...
      try {
        ...
        channels.getChannel().emit(Events.STORY_RENDERED);
      } catch (ex) {
        showException(ex);
      }
    }
