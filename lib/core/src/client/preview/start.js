      ...
      Object.keys(exports).forEach(key => {
        if (isExportStory(key, meta)) {
          ...
          if (parameters && parameters.decorators) {
            deprecate(() => {},
            `${kindName} => ${parameters.displayName || name || key}: story.parameters.decorators is deprecated; use story.decorators instead.`)();
          }
          const decoratorParams = decorators ? { decorators } : null;
          const displayNameParams = name ? { displayName: name } : {};
          const { consistentNames } = kind.getMergedParameters(parameters);
          const storyName = (consistentNames && name) || storyNameFromExport(key);
