...
export default async ({
  ...
  fse.ensureDir(path.resolve(process.cwd(), outputDir));
  entries.forEach((entryFilename, index) => {
    ...
    if (match) {
      ...
      const content = interpolate(entryTemplate, {
        configFilename,
        clientApi,
        clientLogger,
      });
      if (writeFiles) {
        const filename = path.resolve(process.cwd(), outputDir, `${index}.js`);
        fse.writeFileSync(filename, content);
        entries.splice(entries.indexOf(entryFilename), 1, filename);
      } else {
        virtualModuleMapping[entryFilename] = content;
      }
    }
  });
  ...
  if (stories) {
    ...
    const content = interpolate(storyTemplate, { framework })
      // Make sure we also replace quotes for this one
      .replace("'{{stories}}'", stories.map(toRequireContextString).join(','));

    if (writeFiles) {
      fse.writeFileSync(storiesFilename, content);
    } else {
      virtualModuleMapping[storiesFilename] = content;
    }
  }
