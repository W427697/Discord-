...
import { getApp, getServer, getPort } from './utils/http';
import { extend } from './utils/websocket';
...
export async function buildDevStandalone(options) {
  try {
    const { host, packageJson, versionUpdates } = options;
    const { version } = packageJson;

    const [port, updateInfo] = await Promise.all([
    ...
    app.get(/\/static\/media\/.*\..*/, (request, response, next) => {
    ...
    app.get('/', (request, response, next) => {
      if (ready) {
        ...
        response.sendFile(path.join(__dirname, 'templates', 'loading.html'));
      }
    });
    app.get('/loading', (request, response, next) => {
      response.sendFile(path.join(__dirname, 'templates', 'loading.html'));
    });
    app.get('/storybook-logo.svg', (request, response, next) => {
      response.sendFile(path.join(__dirname, 'public', 'storybook-logo.svg'));
    });

    await applyStatic(app, options);

    webEmitter('static applied');
    ...
    const {
      ...
      managerTotalTime,
      previewTotalTime,
    } = await storybook(options, webEmitter);

    webEmitter('storybook ready');
