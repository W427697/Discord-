...
import { getStorybookConfigs } from '@storybook/config/create';
import { start } from '@storybook/serve';

import storybook from './dev-server';
import { getDevCli } from './cli';
import { resolvePathInStorybookCache } from './utils/resolve-path-in-sb-cache';

import { cleanCliOptions } from './utils/cleanCliOptions';
import { cleanEnvOptions } from './utils/cleanEnvOptions';
...
export async function buildDev({ packageJson, ...callOptions }) {
  ...
  logger.info('resolving configs');

  const envOptions = cleanEnvOptions(process.env);
  const cleanedCliOptions = cleanCliOptions(cliOptions);

  const configFiles = await getStorybookConfigs({
    callOptions,
    cliOptions: cleanedCliOptions,
    envOptions,
  });

  if (configFiles) {
    return start({ configFiles, callOptions, cliOptions: cleanedCliOptions });
  }
