 ...
 */
export const resolveAddonName = (name, { storybookMetadata }) => {
  const { path } = parsePackageName(name);

  // when user provides full path, we don't need to look up the package metadata
  if (path) {
    return {
      name,
  ...
  // when user provides a package, either the package declares metadata
  // OR the package is just a preset
  const packageMetadata = storybookMetadata && storybookMetadata[name];
  const { preset, register } = (packageMetadata && packageMetadata.storybook) || {};

  if (preset || register) {
    const entryPath = preset || register;
    return {
      name: entryPath.charAt(0) === '.' ? join(name, entryPath) : entryPath,
      type: register ? 'managerEntries' : 'presets',
    };
  }
  // no package metadata
...
export const splitAddons = (addons, storybookOptions = {}) => {
  return addons.reduce(
    (acc, item) => {
      try {
        if (isObject(item)) {
          const { name } = resolveAddonName(item.name, storybookOptions);
          acc.presets.push({ ...item, name });
        } else {
          const { name, type } = resolveAddonName(item, storybookOptions);
          acc[type].push(name);
        }
      } catch (err) {
        logger.error(
...
function loadPreset(input, level, storybookOptions) {
  try {
    ...
    if (isObject(contents)) {
      ...
      const { managerEntries, presets } = splitAddons(subAddons, storybookOptions);
