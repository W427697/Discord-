...
import gatsbyGenerator from '../generators/GATSBY';
...
const runGenerator = () => {
  switch (projectType) {
    ...
    case types.GATSBY:
      return gatsbyGenerator(npmOptions)
        .then(commandLog('Adding storybook support to your "Gatsby" app'))
        .then(end)
        .then(() => {
          logger.log(
            chalk.yellow(
              'NOTE: Be careful that the specific version 7.1.1 of babel-loader was installed, as this is currently required for Storybook and Gatsby to work together.'
            )
          );
          logger.log();
        });
