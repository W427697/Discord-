...
import surplusGenerator from '../generators/SURPLUS';
...
const installStorybook = (projectType, options) => {
  ...
  const runGenerator = () => {
    switch (projectType) {
      ...
      case types.SURPLUS:
        return surplusGenerator(npmOptions, generatorOptions)
          .then(commandLog('Adding storybook support to your "Surplus" app'))
          .then(end);
