...
type ComputeProps<PropertyName extends string> = PropertyName extends keyof CSSProperties
  ? CSSProperties[PropertyName]
  : string;

type ComputeConditionalProps<Conditions extends BaseConditions, PropertyName extends string> =
  PropertyName extends keyof CSSProperties
    ? Partial<Record<keyof Conditions, CSSProperties[PropertyName]>> | CSSProperties[PropertyName]
    : Partial<Record<keyof Conditions, string>> | string;

export type ComputeFillingsProps<Config> = Config extends ConditionalFillingsConfig<
  infer Conditions,
  infer PropertyName
>
  ? Partial<Record<PropertyName, ComputeConditionalProps<Conditions, PropertyName>>>
...
export type FillingsFn<Config> = Config extends ConditionalFillingsConfig<BaseConditions, string>
  ? (props: ComputeFillingsProps<Config>) => FillingsReturn & { properties: Config['properties'] }
