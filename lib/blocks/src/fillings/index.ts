...
export * from './createRuntimeFillings';
...
export function createFillings<
  ...
  DefaultCondition extends keyof Conditions | Array<keyof Conditions> | false
>(
  options: CreateConditionalFillingsOptions<Property, Conditions, DefaultCondition>
...
export function createFillings(options: any) {
  ...
  if (options.conditions && options.defaultCondition) {
    ...
    const definitions: ConditionalFillingsConfig<BaseConditions, string>['definitions'] = {};
    ...
    for (const property of properties) {
      ...
      for (const conditionName in conditions) {
        ...
        const className = style(styleValue, `fillings_${property}_${conditionName}`);
    ...
    return addFunctionSerializer(createRuntimeFillings(config), {
      importPath: '@storybook/blocks/src/fillings',
