  ...
  function convertToModuleExports(path, originalExports) {
    ...
    adds.forEach(add => {
      ...
      if (add.node.arguments.length > 2) {
        ...
        let { storyParams, storyDecorators } = extractDecorators(originalStoryParams);
        if (name) {
          storyParams = storyParams || j.objectExpression([]);
          storyParams.properties.push(
            j.property('init', j.identifier('consistentNames'), j.literal(true))
          );
        }
