...
module.exports = function transformVueSfc(value) {
  ...
  const storyTemplate = parsedSfcCode.template
    && `template: \`${parsedSfcCode.template.content}\`,`
  const scriptMatch = parsedSfcCode.script
    && SCRIPT_REGEX.exec(parsedSfcCode.script.content)
...
`
  ...
  if (vueSfc) {
    ...
    const sources = Array.isArray(vueSfc.source)
      ? vueSfc.source : [vueSfc.source]
