...
- [Prepare `next` PR](../.github/workflows/prepare-next-release.yml)
- [Prepare hotfix PR](../.github/workflows/prepare-hotfix-release.yml)
...
> **Note**
> Workflow: [`prepare-next-release.yml`](../.github/workflows/prepare-next-release.yml)

`next`-releases are prepared with all content from the `next` branch. The changelog is generated by examining the git history, and looking up all the commits and pull requests between the current prerelease (on `next-release`) and `HEAD` of `next`.
...
> **Note**
> Workflow: [`prepare-hotfix-release.yml`](../.github/workflows/prepare-hotfix-release.yml)

Hotfix releases are created by [cherry-picking](https://www.atlassian.com/git/tutorials/cherry-pick) any merged, unreleased pull requests that have the "**patch:yes**" label applied to the `next` branch. The merge commit of said pull requests are cherry-picked.
...
When either a `next`-release or a hotfix release branch is merged into `latest-release` or `next-release`, the publishing workflow is triggered. This workflow performs the following tasks:
...
4. (If this is a hotfix release, add the "**patch:done**" label to all relevant pull requests.)
...
7. (If this is a hotfix release, copy the `CHANGELOG.md` changes from `main` to `next`.)
...
This is the default strategy for `next`-releases, there's nothing special needed to trigger this scenario.
...
This is the default hotfix release scenario, which cherry picks patches to `main`.
...
You can always see the workflows and if they have been cancelled [here for `next`-releases](https://github.com/storybookjs/storybook/actions/workflows/prepare-next-release.yml) and [here for hotfix releases](https://github.com/storybookjs/storybook/actions/workflows/prepare-hotfix-release.yml).
