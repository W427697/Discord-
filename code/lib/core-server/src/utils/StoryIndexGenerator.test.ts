...
import type { StoryIndexer, StoryIndexEntry } from '@storybook/types';
...
describe('StoryIndexGenerator', () => {
  ...
  describe('extraction', () => {
    const storiesSpecifier = normalizeStoriesEntry('./src/A.stories.(ts|js|jsx)', options);
    const docsSpecifier = normalizeStoriesEntry('./src/docs2/*.mdx', options);

    describe('single file specifier', () => {
      it('extracts stories from the right files', async () => {
        const specifier = await normalizeStoriesEntry('./src/A.stories.js', options);
    ...
    describe('non-recursive specifier', () => {
      it('extracts stories from the right files', async () => {
        const specifier = await normalizeStoriesEntry('./src/*/*.stories.(ts|js|jsx)', options);
    ...
    describe('mdx tagged components', () => {
      it('adds docs entry with docs enabled', async () => {
        const specifier = await normalizeStoriesEntry('./src/nested/Page.stories.mdx', options);
    ...
    describe('autodocs', () => {
      ...
      it('throws an error if you attach a MetaOf entry to a tagged autodocs entry', async () => {
        const csfSpecifier = await normalizeStoriesEntry('./src/B.stories.ts', options);

        const docsSpecifier = await normalizeStoriesEntry('./errors/MetaOfAutodocs.mdx', options);
      ...
      it('allows you to override autodocs with MetaOf if it is automatic', async () => {
        const csfSpecifier = await normalizeStoriesEntry('./src/A.stories.js', options);

        const docsSpecifier = await normalizeStoriesEntry('./src/docs2/MetaOf.mdx', options);
      ...
      it('generates a combined entry if there are two stories files for the same title', async () => {
        const specifier = await normalizeStoriesEntry('./duplicate/*.stories.(ts|js|jsx)', options);
      ...
      it('does not generate a docs page entry if there are no stories in the CSF file', async () => {
        const csfSpecifier = await normalizeStoriesEntry('./errors/NoStories.stories.ts', options);
    ...
    describe('docs specifier', () => {
      it('creates correct docs entries', async () => {
        const generator = new StoryIndexGenerator(
          [await storiesSpecifier, await docsSpecifier],
          options
        );
        await generator.initialize();
      ...
      it('generates no docs entries when docs are disabled', async () => {
        const generator = new StoryIndexGenerator([await storiesSpecifier, await docsSpecifier], {
          ...options,
          docs: {
            ...options.docs,
            disable: true,
    ...
    describe('errors', () => {
      it('when docs dependencies are missing', async () => {
        const generator = new StoryIndexGenerator(
          [await normalizeStoriesEntry('./src/docs2/MetaOf.mdx', options)],
          options
        );
    ...
    describe('duplicates', () => {
      it('warns when two MDX entries reference the same CSF file without a name', async () => {
        const docsErrorSpecifier = await normalizeStoriesEntry(
          './errors/DuplicateMetaOf.mdx',
          options
        );

        const generator = new StoryIndexGenerator(
          [await storiesSpecifier, await docsSpecifier, docsErrorSpecifier],
          options
        );
        await generator.initialize();
  ...
  describe('sorting', () => {
    it('runs a user-defined sort function', async () => {
      const storiesSpecifier = await normalizeStoriesEntry(
        './src/**/*.stories.(ts|js|jsx)',
        options
      );
      const docsSpecifier = await normalizeStoriesEntry('./src/docs2/*.mdx', options);
  ...
  describe('caching', () => {
    ...
    describe('file changed', () => {
      it('calls extract csf file for just the one file', async () => {
        ...
        await generator.invalidate(specifier, './src/B.stories.ts', false);
      ...
      it('calls extract docs file for just the one file', async () => {
        ...
        await generator.invalidate(docsSpecifier, './src/docs2/Title.mdx', false);
      ...
      it('calls extract for a csf file and any of its docs dependents', async () => {
        ...
        await generator.invalidate(storiesSpecifier, './src/A.stories.js', false);
    ...
    describe('file removed', () => {
      it('does not extract csf files a second time', async () => {
        ...
        await generator.invalidate(specifier, './src/B.stories.ts', true);
      ...
      it('does not include the deleted docs in results', async () => {
        ...
        await generator.invalidate(docsSpecifier, './src/docs2/NoTitle.mdx', true);
      ...
      it('cleans up properly on dependent docs deletion', async () => {
        ...
        await generator.invalidate(docsSpecifier, './src/docs2/MetaOf.mdx', true);
