...
export const PLAY_FN_TAG = 'sb:play-fn';
 ...
 *   - If the stories have the 'sb:stories-mdx' tag (i.e. were generated by a .stories.mdx file),
 ...
 *
 * In the preview, a docs entry with either the `autodocs` or `sb:stories-mdx` tags will be rendered
  ...
  async extractStories(
    ...
    const entries: ((StoryIndexEntryWithMetaId | DocsCacheEntry) & { tags: Tag[] })[] =
      indexInputs.map((input) => {
        ...
        const tags = (input.tags || []).concat(STORY_TAG);
    ...
    if (createDocEntry) {
      ...
      entries.unshift({
        ...
        importPath,
        type: 'docs',
        tags: [...tags, DOCS_TAG, ...(!hasAutodocsTag && !isStoriesMdx ? [AUTODOCS_TAG] : [])],
        storiesImports: [],
      });
    }

    const entriesWithoutDocsOnlyStories = entries.filter(
      (entry) => !(entry.type === 'story' && entry.tags.includes('sb:stories-mdx-docsOnly'))
    );
  ...
  async extractStoriesFromDeprecatedIndexer({
    ...
    csf.stories.forEach(({ id, name, tags: storyTags, parameters }) => {
      if (!parameters?.docsOnly) {
        const tags = (csf.meta.tags ?? []).concat(storyTags ?? [], STORY_TAG);
