/* eslint-disable @typescript-eslint/no-non-null-assertion, @typescript-eslint/ban-types, func-names, @typescript-eslint/no-shadow, no-param-reassign, no-restricted-syntax, no-return-assign, no-inner-declarations, consistent-return, no-plusplus, no-underscore-dangle, prefer-rest-params */
...
/**
 * We copied it because because of the dist of `jest-mock` package is CJS and can't be tree-shaken effectively,
 * This causes node dependencies to leak into the browser bundle.
 * @see: https://github.com/storybookjs/storybook/issues/23570
 *
 * this code is copied from https://github.com/jestjs/jest/blob/main/packages/jest-util/src/isPromise.ts
 * this code is copied from https://github.com/jestjs/jest/blob/main/packages/jest-mock/src/index.ts
 *
 * I made changes to the source files:
 * 1. remove all `export` keywords, except for `class ModuleMocker`
 * 2. remove unused variables/interfaces/types
 */
...
function matchArity(fn: Function, length: number): Function {
  ...
  switch (length) {
    ...
    case 2:
      ...
      break;
    case 3:
      mockConstructor = function (this: unknown, _a: unknown, _b: unknown, _c: unknown) {
        return fn.apply(this, arguments);
      };
      break;
    case 4:
...
export class ModuleMocker {
  ...
  private _ensureMockState<T extends UnknownFunction>(f: Mock<T>): MockFunctionState<T> {
  ...
  private _makeComponent<T extends UnknownFunction>(
    ...
    if (metadata.type === 'function') {
      const prototype =
        (metadata.members && metadata.members.prototype && metadata.members.prototype.members) ||
        {};
      const prototypeSlots = this._getSlots(prototype);
      // eslint-disable-next-line @typescript-eslint/no-this-alias
      const mocker = this;
      const mockConstructor = matchArity(function (this: ReturnType<T>, ...args: Parameters<T>) {
      ...
      f.mockResolvedValueOnce = (value: ResolveType<T>) =>
        f.mockImplementationOnce(() => this._environmentGlobal.Promise.resolve(value));

      f.mockRejectedValueOnce = (value: unknown) =>
        f.mockImplementationOnce(() => this._environmentGlobal.Promise.reject(value));
      ...
      f.mockResolvedValue = (value: ResolveType<T>) =>
        f.mockImplementation(() => this._environmentGlobal.Promise.resolve(value));

      f.mockRejectedValue = (value: unknown) =>
        f.mockImplementation(() => this._environmentGlobal.Promise.reject(value));
      ...
      function withImplementation(fn: T, callback: () => Promise<unknown>): Promise<void>;
  ...
  private _generateMock<T>(
    ...
    refs: Record<number, Record<string, any> | Array<unknown> | RegExp | T | Mock | undefined>
  ): Mocked<T> {
  ...
  private _attachMockImplementation<T extends Function>(mock: Mock<UnknownFunction>, original: T) {
  ...
  spyOn<
    ...
    V extends Required<T>[K]
  >(object: T, methodKey: K): V extends ClassLike | FunctionLike ? Spied<V> : never;

  spyOn<T extends object>(object: T, methodKey: keyof T, accessType?: 'get' | 'set'): MockInstance {
      ...
      if (typeof original !== 'function') {
        throw new Error(
          `Cannot spy on the \`${String(
            methodKey
          )}\` property because it is not a function; ${this._typeOf(original)} given instead.${
  ...
  replaceProperty<T extends object, K extends keyof T>(
    ...
    if (object == null || (typeof object !== 'object' && typeof object !== 'function')) {
      throw new Error(
        `Cannot use replaceProperty on a primitive value; ${this._typeOf(object)} given`
      );
    }
  ...
  mocked<T extends object>(source: T, options: { shallow: true }): MockedShallow<T>;
