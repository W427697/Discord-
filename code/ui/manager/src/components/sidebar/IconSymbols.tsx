...
const FolderPath = styled.path(({ theme }) => ({
  fill: theme.base === 'dark' ? theme.color.primary : theme.color.ultraviolet,
}));

const ComponentPath = styled.path(({ theme }) => ({
  fill: theme.color.secondary,
}));

const DocumentPath = styled.path(({ theme }) => ({
  fill: theme.base === 'dark' ? theme.color.gold : '#ff8300',
}));

const StoryPath = styled.path(({ theme }) => ({
  fill: theme.color.seafoam,
}));
  ...
  return (
    <Svg data-chromatic="ignore">
      ...
      <symbol id="icon--document">
        ...
        <DocumentPath d="M4 5.5a.5.5 0 01.5-.5h5a.5.5 0 010 1h-5a.5.5 0 01-.5-.5zM4.5 7.5a.5.5 0 000 1h5a.5.5 0 000-1h-5zM4 10.5a.5.5 0 01.5-.5h5a.5.5 0 010 1h-5a.5.5 0 01-.5-.5z" />
        <DocumentPath
          fillRule="evenodd"
          clipRule="evenodd"
